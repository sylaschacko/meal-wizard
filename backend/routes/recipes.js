...
router.get('/fetch', async (req, res) => {
  try {
    const response = await axios.get(`https://api.spoonacular.com/recipes/random?number=10&apiKey=${process.env.SPOONACULAR_API_KEY}`);
    const recipes = response.data.recipes;

    // Save recipes to MongoDB
    const savedRecipes = await Promise.all(recipes.map(async (recipeData) => {
      const existingRecipe = await Recipe.findOne({ spoonacularId: recipeData.id });
...
router.get('/search', async (req, res) => {
  const { ingredients, excludeIngredients, maxCalories, dietaryPreferences, cuisine, maxCookingTime } = req.query;
  try {
    ...
    if (dietaryPreferences) {
      ...
      recipes = recipes.filter(recipe =>
        preferencesArray.every(pref => recipe.dietaryPreferences.includes(pref))
      );
    }
